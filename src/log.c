 ...
 */
static void
log_enq(log_q_t *queue, arena_t *arena, const char *msg)
{
	assert(queue != NULL);
	assert(arena != NULL);
	assert(msg != NULL);
 ...
 */
int
init_log_system(log_q_t *queue, arena_t *arena, FILE *log_file)
{
	assert(queue != NULL);
 ...
 */
void
log_message_internal(log_q_t *queue,
                     arena_t *arena,
                     log_level_e level,
                     const char *file,
                     int line,
