 ...
 */
int
cooper_shm_init_agent(cooper_shm_context_t *ctx)
{
	assert(ctx != NULL);
	...
	if (ctx->data_fd == -1)
	{
		LOG_ERROR("shm_open failed for %s: %s (errno=%d)",
		          COOPER_DATA_SHM_NAME,
		          strerror(errno),
		          errno);
		return -1;
	}
 ...
 */
void
cooper_shm_cleanup_read_entries(cooper_shm_context_t *ctx)
{
	assert(ctx != NULL);
 ...
 */
int
cooper_shm_cleanup_agent(cooper_shm_context_t *ctx)
{
	assert(ctx != NULL);
