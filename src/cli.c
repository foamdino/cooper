...
init_cli_shm()
{
	...
	shm_ctx.data_shm = mmap(
	    NULL, sizeof(cooper_data_shm_t), PROT_READ, MAP_SHARED, shm_ctx.data_fd, 0);
	...
	shm_ctx.status_shm = mmap(NULL,
	                          sizeof(cooper_status_shm_t),
	...
	for (uint32_t i = 0; i < COOPER_MAX_ENTRIES; i++)
	{
		...
		switch (entry->type)
		{
			case COOPER_DATA_METHOD_METRIC: {
				...
				if (method_idx == -1
				    && method_count < UI_MAX_DISPLAY_ITEMS)
					method_idx = method_count++;

				if (method_idx >= 0)
				{
					/* Copy signature */
					strncpy(methods[method_idx].signature,
					        method->signature,
					        sizeof(methods[method_idx].signature)
					            - 1);
					methods[method_idx].signature
					    [sizeof(methods[method_idx].signature) - 1] =
					    '\0';
					/* Direct field access */
					methods[method_idx].call_count =
					    method->call_count;
					methods[method_idx].sample_count =
					    method->sample_count;
					methods[method_idx].total_time_ns =
					    method->total_time_ns;
					methods[method_idx].min_time_ns =
					    method->min_time_ns;
					methods[method_idx].max_time_ns =
					    method->max_time_ns;
					methods[method_idx].alloc_bytes =
					    method->alloc_bytes;
					methods[method_idx].peak_memory =
					    method->peak_memory;
					methods[method_idx].cpu_cycles =
					    method->cpu_cycles;
			...
			case COOPER_DATA_MEMORY_SAMPLE: {
				...
				if (memory->thread_id == 0)
				{
					...
					/* Add to history */
					if (memory_data.history_count
					    < UI_MAX_HISTORY_POINTS)
					{
						memory_data.memory_history
						    [memory_data.history_count++] =
						    memory->process_memory;
					}
					else
					{
						/* Shift history */
						for (int j = 0;
						     j < UI_MAX_HISTORY_POINTS - 1;
						     j++)
						{
							memory_data.memory_history[j] =
							    memory_data
								.memory_history[j + 1];
						}
						memory_data
						    .memory_history[UI_MAX_HISTORY_POINTS
						                    - 1] =
						    memory->process_memory;
					}
				}
				else
				{
					...
					int thread_idx = -1;
					for (int j = 0; j < memory_data.active_threads;
					     j++)
					{
						if (memory_data.thread_ids[j]
						    == memory->thread_id)
						{
							thread_idx = j;
							break;
					...
					if (thread_idx == -1
					    && memory_data.active_threads < 10)
					{
						thread_idx = memory_data.active_threads++;
						memory_data.thread_ids[thread_idx] =
						    memory->thread_id;
					}

					if (thread_idx >= 0)
						memory_data.thread_memory[thread_idx] =
			...
			case COOPER_DATA_OBJECT_ALLOC: {
				...
				{
					if (strcmp(objects[j].class_name,
					           alloc->class_signature)
					    == 0)
					{
						obj_idx = j;
						break;
				...
				if (obj_idx >= 0)
				{
					...
					strncpy(objects[obj_idx].class_name,
					        alloc->class_signature,
					        sizeof(objects[obj_idx].class_name) - 1);
					objects[obj_idx].class_name
					    [sizeof(objects[obj_idx].class_name) - 1] =
					    '\0';

					/* Semantic field names */
					objects[obj_idx].allocation_count =
					    alloc->allocation_count;
					objects[obj_idx].current_instances =
					    alloc->current_instances;
					objects[obj_idx].total_bytes = alloc->total_bytes;
					objects[obj_idx].peak_instances =
					    alloc->peak_instances;
...
debug_shared_memory()
{
	...
	printf("DEBUG: SHM Status - version=%u, max_entries=%u, next_write=%u\n",
	       shm_ctx.data_shm->version,
	       shm_ctx.data_shm->max_entries,
