...
int
cache_init_system(arena_t *arena)
{
	if (cache_arena != NULL)
		return 0;
...
static void
default_copy(void *dest, const void *src, size_t size)
{
	memcpy(dest, src, size);
}

cache_t *
cache_init(arena_t *arena, const cache_config_t *config)
{
	assert(arena != NULL);
	assert(config != NULL);

	if (!arena || !config || config->max_entries == 0 || config->key_size == 0
	    || config->value_size == 0 || !config->key_compare)
	{
		return NULL;
	}
 ...
 */
int
cache_get(cache_t *cache, const void *key, void *value)
{
	if (!cache || !key)
		return 1;
	...
	for (size_t i = 0; i < cache->capacity; i++)
	{
		    ...
		    && cache->config.key_compare(cache->entries[i].key, key) == 0)
		{
			...
			if (value)
				cache->config.value_copy(value,
				                         cache->entries[i].value,
				                         cache->config.value_size);
 ...
 */
int
cache_put(cache_t *cache, const void *key, const void *value)
{
	if (!cache || !key || !value)
		return 1;
	...
	for (size_t i = 0; i < cache->capacity; i++)
	{
		    ...
		    && cache->config.key_compare(cache->entries[i].key, key) == 0)
		{
			...
			cache->config.value_copy(
			    cache->entries[i].value, value, cache->config.value_size);
			return 0;
		}
	}
	...
	cache->config.key_copy(
	    cache->entries[target_index].key, key, cache->config.key_size);
	cache->config.value_copy(
	    cache->entries[target_index].value, value, cache->config.value_size);

	return 0;
}

void
cache_clear(cache_t *cache)
{
...
void
cache_stats(cache_t *cache, size_t *hits, size_t *misses, size_t *entries)
{
	if (!cache)
		return;
	...
	/* Note: Individual cache instances don't track hits/misses to keep them
	   lightweight. Hit/miss tracking is done at the thread level by the TLS manager.
	 */
	if (hits)
		*hits = 0;
...
cache_t *
cache_tls_get(const char *cache_id, arena_t *data_arena, const cache_config_t *config)
{
	...
	strncpy(manager->caches[index].cache_id,
	        cache_id,
	        sizeof(manager->caches[index].cache_id) - 1);
	manager->caches[index].cache_id[sizeof(manager->caches[index].cache_id) - 1] =
	    '\0';
