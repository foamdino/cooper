...
rdtsc_start(void)
{
	...
	*/
	...
	asm volatile(
	    ...
	    "mov %%edx, %0\n\t" /* move the content of edx into the first output variable
	                           - cycle_high_bits */
	    "mov %%eax, %1\n\t" /* move the content of eax into the second output variable
	                           - cycle_low_bits */
	    : "=r"(cycles_high_bits), "=r"(cycles_low_bits) /* Output operands */
	    :                                               /* no input operands */
	    : "%rax", "%rbx", "%rcx", "%rdx"); /* List of 'clobbered' registers  (CPUID
	                                          modifies the 64bit version rax etc)*/
