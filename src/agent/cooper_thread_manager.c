...
start_all_threads(agent_context_t *ctx)
{
	...
	if (pthread_create(&ctx->mem_sampling_thread, NULL, mem_sampling_thread_func, ctx)
	    != 0)
	{
	...
	if (pthread_create(&ctx->heap_stats_thread, NULL, heap_stats_thread_func, ctx)
	    != 0)
	{
	...
	if (ctx->shm_ctx != NULL)
	{
		...
		else
		{
			...
			if (pthread_create(&ctx->shm_export_thread,
			                   NULL,
			    ...
			    != 0)
			{
				LOG_ERROR("Failed to start shm export thread: %s",
				          strerror(errno));
				clear_worker_status(&ctx->worker_statuses,
				                    SHM_EXPORT_RUNNING);
				return COOPER_ERR;
			}
			else
